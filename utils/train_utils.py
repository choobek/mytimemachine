...
def compute_cosine_weights(x):
	...
	assert np.min(values) >= 0. and np.max(values) <= 1., f"Input values should be between 0. and 1! Got {values}"
	weights = 0.25 * (np.cos(np.pi * values)) + 0.75
	return weights

def compute_cosine_weights_reverse(x):
	values = np.abs(x.cpu().detach().numpy())
	assert np.min(values) >= 0. and np.max(values) <= 1., "Input values should be between 0. and 1!"
	# when values=0, return 0, when values=1, return 1
	weights = 0.5 * (np.cos(np.pi * values - np.pi)) + 0.5
	return weights
