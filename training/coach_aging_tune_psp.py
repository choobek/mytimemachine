		...
		if self.opts.train_encoder and self.global_step <= self.encoder_step:
			...
			if (not no_aging) and (not self.interpolation):
				...
				if self.global_step % 50 == 0:
					...
					reconstructed.save(os.path.join(debug_dir, f'{self.global_step}_{target_age:.4f}_reconstructed_face_{data_type}.png'))
					# save the most similar face
					reconstructed_global = common.tensor2im(y_global[0])
					reconstructed_global.save(os.path.join(debug_dir, f'{self.global_step}_{target_age:.4f}_reconstructed_face_global_{data_type}.png'))
