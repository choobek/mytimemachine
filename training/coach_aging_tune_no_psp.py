...
class Coach:
	...
	def perform_forward_pass_blender(self, x):
		...
		target_ages = x[:, -1, 0, 0]
		latent_blended = self.net.blender(latent_local, latent_global, target_ages, self.global_step)
		...
		return y_hat, latent_blended, latent_local_, latent_global_
	...
	# 			_, latent_global_age = self.net_global.forward(x_input, return_styles=True)
	# 			_, latent_local_age = self.net.forward(x_input, return_styles=True)
	...
	# 	latent_blended = self.net.blender(latent_local, latent_global, latent_local_ages, latent_global_ages, target_ages)
	# 	y_hat, _ = self.net.decoder(
	# 		[latent_blended], 
	# 		input_is_latent=True, 
	# 		randomize_noise=False
		...
		while self.global_step <= self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				if no_aging:
					...
					# choose interpolation unless we sample extrapolation
				...
				self.encoder_step = 1e4
				self.decoder_step = self.encoder_step + 1e4

				if self.opts.train_encoder and self.global_step <= self.encoder_step:
					# todo
					naive_personalization = True
					if naive_personalization:
						...
						y_hat_inverse = torch.stack(y_hat_inverse)
						reverse_target_ages = y_hat_inverse[:, -1, 0, 0]
						y_recovered, latent_cycle, latent_local, latent_global = self.perform_forward_pass_blender(y_hat_inverse)
						loss, cycle_loss_dict, cycle_id_logs = self.calc_loss(x, y, y_recovered, latent_cycle,
																			target_ages=reverse_target_ages,
						...
						for idx, cycle_log in enumerate(cycle_id_logs):
							id_logs[idx].update(cycle_log)
						loss_dict.update(cycle_loss_dict)
						loss_dict["loss"] = loss_dict["loss_real"] + loss_dict.get("loss_cycle", 0)

						# Gradient clipping + NaN guard for blender
				...
				if self.opts.train_decoder and self.encoder_step < self.global_step <= self.decoder_step:
					...
					# Gradient clipping + NaN guard for decoder
	...
	def _build_scheduler(self, optimizer):
		"""Cosine decay with warmup if enabled via opts.scheduler_type."""
		...
		# capture base lr per group
		for group in optimizer.param_groups:
			group.setdefault('initial_lr', group.get('lr', 1e-8))
		def lr_lambda(step):
			# piecewise: linear warmup, then cosine to min_lr
			if max_steps <= 0:
				return 1.0
			...
			# compute ratio using first group's base
	...
	def _clip_and_step(self, optimizer, params_iter):
		"""Clip gradients if enabled; guard against NaN/Inf; step optimizer if ok.
		Returns True if step performed, else False."""
	...
	def calc_loss(self, x, y, y_hat, latent, target_ages, input_ages, no_aging, data_type="real", latent_local=None, latent_global=None):
		...
		if self.opts.train_encoder and self.global_step <= self.encoder_step:
			...
			# if self.opts.w_norm_lambda > 0:
			# 	loss_w_norm = self.w_norm_loss(latent, latent_avg=latent_global)
			# 	loss_dict[f'dilation_loss'] = float(loss_w_norm)
			# 	loss += loss_w_norm * self.opts.w_norm_lambda
			...
			# 	loss_dict[f'additional_w_norm_{data_type}'] = float(loss_w_norm)
			# 	loss += loss_w_norm * self.opts.w_norm_lambda

			# l2 delta_regularizations loss similar to e4e for better editing
			# delta_w = latent - latent_global
			# loss_l2_delta_e4e = F.mse_loss(delta_w, torch.zeros_like(delta_w))
			# loss_dict[f'loss_l2_delta_e4e'] = float(loss_l2_delta_e4e)
			# print('loss_l2_delta_e4e:', loss_l2_delta_e4e.item())
			# loss += loss_l2_delta_e4e * 1e-5


			
			# # delta_w should be perpendicular to global_w
			# delta_w = latent - latent_global
			# # latents are of shape (b, 18, 512)
			# # orthogonal loss is the sum of squared inner products between delta_w and global_w
			# loss_ortho = ((delta_w * latent_global).sum(-1) ** 2).sum(dim=1).mean()
			# loss_dict[f'loss_ortho'] = float(loss_ortho)
			# loss += loss_ortho * 1e-5
			...
			if (not no_aging) and self.interpolation:
				...
				loss += nearest_neighbor_id_loss * self.opts.nearest_neighbor_id_loss_lambda
			...
			if (not no_aging) and (not self.interpolation):
				...
				y_local, latent_local = self.net.forward(x_input, return_latents=True)
				loss = torch.tensor(0.0).to(self.device)
				...
				if self.opts.id_lambda > 0:
					...
					if self.opts.use_weighted_id_loss:  # compute weighted id loss only on forward pass
						...
						weights = train_utils.compute_cosine_weights(x=age_diffs)
						# weights = train_utils.compute_cosine_weights_reverse(x=age_diffs)
						# print('sanity check for cosine reverse weights:')
						# print('age_diffs:', age_diffs)
				...
				if self.opts.l2_lambda > 0:
					...
					# 	loss_l2 = F.mse_loss(y_local[i], y_global[i])
					# 	loss_l2 = loss_l2 * weights[i]
					# 	l2_lambda = self.opts.l2_lambda
					# 	extrapolation_loss += loss_l2 * l2_lambda
				if self.opts.lpips_lambda > 0:
					...
					# for i in range(len(y_local)):
					# 	loss_lpips = self.lpips_loss(y_local[i], y_global[i])
					# 	loss_lpips = loss_lpips * weights[i]
					# 	lpips_lambda = self.opts.lpips_lambda
					# 	extrapolation_loss += loss_lpips * lpips_lambda
				if self.opts.lpips_lambda_crop > 0:
					...
					# for i in range(len(y_local)):
					# 	loss_lpips_crop = self.lpips_loss(y_local[i].unsqueeze(0)[:, :, 35:223, 32:220], y_global[i].unsqueeze(0)[:, :, 35:223, 32:220])
					# 	loss_lpips_crop = loss_lpips_crop * weights[i]
					# 	extrapolation_loss += loss_lpips_crop * self.opts.lpips_lambda_crop
				if self.opts.l2_lambda_crop > 0:
					...
					# for i in range(len(y_local)):
					# 	loss_l2_crop = F.mse_loss(y_local[i].unsqueeze(0)[:, :, 35:223, 32:220], y_global[i].unsqueeze(0)[:, :, 35:223, 32:220])
					# 	loss_l2_crop = loss_l2_crop * weights[i]
					# 	extrapolation_loss += loss_l2_crop * self.opts.l2_lambda_crop
				# weights = train_utils.compute_cosine_weights(x=age_diffs)
				...
				if self.opts.w_norm_lambda > 0:
					# loss_w_norm = self.w_norm_loss(latent, latent_avg=self.net.latent_avg)
					# loss_dict[f'loss_w_norm_{data_type}'] = float(loss_w_norm)
					# extrapolation_loss += loss_w_norm * self.opts.w_norm_lambda * 5
					# todo: make w_norm_lambda adaptive based on age_diffs
					loss_w_norm = torch.tensor(0.0).to(self.device)
					age_diffs = torch.abs(target_ages - input_ages)
					weights = (1 - train_utils.compute_cosine_weights(x=age_diffs)) * self.opts.adaptive_w_norm_lambda 
					for i in range(len(latent)):
				...
				if self.global_step % 50 == 0:
					...
					os.makedirs(debug_dir, exist_ok=True)
					# target_age = target_ages[0].item()
					target_age = x_input[:, -1, 0, 0][0].item()
					# save input face
					input_face = common.tensor2im(x[0])
					...
					reconstructed.save(os.path.join(debug_dir, f'{self.global_step}_target_age_{target_age:.4f}_reconstructed_face_{data_type}.png'))
					# save the most similar face
					reconstructed_global = common.tensor2im(y_global[0])
					reconstructed_global.save(os.path.join(debug_dir, f'{self.global_step}_target_age_{target_age:.4f}_reconstructed_face_global_{data_type}.png'))
