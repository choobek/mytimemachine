...
class pSp(nn.Module):
	...
	def __init__(self, opts):
		...
		train_x = self.opts.feats_dict.keys()
		train_x = torch.tensor(list(train_x)).float()
		train_y = torch.ones_like(train_x)
		self.blender = Blender(train_x, train_y, self.likelihood)
	...
	def load_weights(self):
		if self.opts.checkpoint_path is not None:
			...
			for k in ckpt['state_dict'].keys():
				if 'blender' in k:
					self.blender.load_state_dict(self.__get_keys(ckpt, 'blender'), strict=True)
	...
	def forward(self, x, resize=True, latent_mask=None, input_code=False, randomize_noise=True,
				inject_latent=None, return_latents=False, alpha=None, input_is_full=False, return_styles=False):
		if input_code:
			codes = x
		else:
